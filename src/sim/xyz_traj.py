...
added function to save data as mat file - AM
"""
...
class Trajectory:
    ...
    def save(self, func):
        import scipy.io
        pts = self.getPList(func, 0.0, self.s_max, self.s_max/1000)
        # scipy.io.savemat('/home/amith/Documents/BYU/VTOL/code/data/xyztraj.mat', mdict={'xr': pts[0,:],'yr':pts[1,:],'zr':pts[2,:]})
        np.save('test_traj.npy', pts)
...
if __name__ == "__main__":
    ...
    traj = XYZSinusoid(150., 150., 75., 600., 300., 600., -np.pi/2, np.pi, 0.*np.pi/2)
    traj.save(traj.getP)